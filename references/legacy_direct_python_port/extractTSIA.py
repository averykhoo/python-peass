...
def extractTSIA(s: np.ndarray, sEst: np.ndarray, flen: int, Lw: int, hop: int, options: dict) -> tuple:
    # %%% MATLAB Code %%%
    ...
    # for nSource = 1:NSources, yProjAll(:,:,nSource) = sum(yProjAll_(1:end-flen+1,:,(nSource-1)*NChan+(1:NChan)),3);
    ...
    flag_2proj = options.get('FLAG_2PROJ', False)
    ...
    # for nEst=1:NEst, sTrue(:,:,nEst) = s(:,1:NChan);
    for nEst in range(NEst):
        start_est = nEst * NChan
        end_est = (nEst + 1) * NChan
        sTrue[:, start_est:end_est] = s_reshaped[:, :NChan]

    # if options.FLAG_2PROJ, eSpat = eSpat-sTrue; else, eSpat = yProjAll(:,:,1:NEst)-sTrue;
    if flag_2proj:
        eSpat = eSpat - sTrue
    else:
