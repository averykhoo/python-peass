...
Equivalent of extractDistortionComponents.m
"""
import os
...
def extractDistortionComponents(srcFiles: list, estFile, options: dict = None, fs: float = None):
    # %%% MATLAB Code %%%
    defaultOptions = {
        'destDir':            './',
        'FLAG_2PROJ':         False,
    ...
    for j, s_data in enumerate(src_data_list):
        if s_data.shape != est_data.shape:
            raise ValueError(f"PEASS:BadSoundSize - Sound files must have the same size.")
