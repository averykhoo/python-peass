...
def test_correctness_on_real_audio_db(db_resources):
    """Verifies subspace decomposition on actual speech files from the DB."""
    target_path = db_resources / "exp01_target.wav"
    interf_path = db_resources / "exp01_InterfSrc1.wav"

    assert target_path.exists(), f"Missing test resource: {target_path}"

    target, fs = sf.read(target_path)
    interferer, _ = sf.read(interf_path)
    ...
    # Verify Interference matching target interferer correlation
    # We use a slightly lower threshold for the broad DB test vs the gold standard
...
def test_regression_against_matlab_references(matlab_ref_resources):
    ...
    for py_val, gold_filename, label in validation_map:
        ...
        # Sync lengths (handling tiny decimation alignment diffs)
